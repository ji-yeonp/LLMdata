...
from network_mae.temporal_former_0_1 import *
...
def train_mae(exp_name):
    ...
    loss_mae_j = AverageMeter()
    loss_mae_m = AverageMeter()
    total_mpjpe_j = AverageMeter()
    total_mpjpe_m = AverageMeter()
    ...
    for i in range(resume_epoch, num_epochs):
        ...
        for phase in ['train', 'valid']:
            ...
            for ii, (joints, bones, motions, labels, _, names, _) in enumerate(trainval_loaders[phase]):
                inputs_1 = joints.to(device).float()
                inputs_2 = motions.to(device).float()
                labels = labels.to(device).float()

                if phase == 'train':
                    _, outputs_1, outputs_2 = model(inputs_1, inputs_2, mask_ratio=mask_ratio)
                    loss_j = criterion(outputs_1, inputs_1)
                    loss_m = criterion(outputs_2, inputs_2)
                    loss = loss_j + loss_m
                    optimizer.zero_grad()
                    loss.backward()
                    optimizer.step()
                ...
                mpjpe_j = torch.norm(inputs_1 - outputs_1, p='fro', dim=-1)
                mpjpe_m = torch.norm(inputs_2 - outputs_2, p='fro', dim=-1)

                loss_mae_j.update(loss_j.item(), n=inputs_1.size(0))
                loss_mae_m.update(loss_m.item(), n=inputs_2.size(0))
                total_mpjpe_j.update(mpjpe_j.mean().item(), n=inputs_1.size(0))
                total_mpjpe_m.update(mpjpe_m.mean().item(), n=inputs_2.size(0))
                name_list.extend(names)

                batch_time.update(time.time() - end)
                end = time.time()

                bar.suffix = f'({ii + 1}/{len(trainval_loaders[phase])}) | Batch: {batch_time.avg:.3f}s | ETA: {bar.eta_td:} | Loss-J/M: {loss_mae_j.avg:.6f} / {loss_mae_m.avg:.6f} | MPJPE-J/M: {1000.0 * total_mpjpe_j.avg:.2f} / {1000.0 * total_mpjpe_m.avg:.2f} mm'
                bar.next()
            bar.finish()
