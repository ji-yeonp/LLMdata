...
def train_model(exp_name, repNo):
    ...
    for i in range(resume_epoch, num_epochs):
        ...
        print(f'\nTrial: {repNo}/{NUM_REPETITIONS} Epoch: {i + 1}/{num_epochs} LR: {curent_lr}')
        ...
        for phase in ['train', 'valid']:
            ...
            label_list = list()
            name_list = list()
            for ii, (joints, bones, motions, music, labels, names) in enumerate(trainval_loaders[phase]):
