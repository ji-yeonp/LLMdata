...
# from network_mae.temporal_former_1 import *
...
# MAE = 4096, fine-tuing = 1024, train-from-scratch = 4096
...
def train_model(exp_name, repNo):
    ...
    for i in range(resume_epoch, num_epochs):
        ...
        for phase in trainval_loaders.keys():
            ...
            name_list = list()
            frame_list = list()
            for ii, (joints, bones, motions, labels, names, frm_index) in enumerate(trainval_loaders[phase]):
            ...
            if phase == 'train':
                ...
                save_resutls(name_list, frame_list, score_list, label_list, os.path.join(EXP_FOLDER, exp_name, f'{repNo:03d}', f'train-results-str{STRIDE}.pkl'))
            elif phase == 'update':
                ...
                # diversity_measure(name_list, score_list, label_list, f'update-measure-all-str{STRIDE}.npy')
            elif phase == 'valid':
                ...
                save_resutls(name_list, frame_list, score_list, label_list, os.path.join(EXP_FOLDER, exp_name, f'{repNo:03d}', f'valid-results-str{STRIDE}.pkl'))
    ...
    return update_barometer, valid_barometer
...
if __name__ == "__main__":
    ...
    update_PLCC_win_list = np.zeros((NUM_REPETITIONS, num_epochs))
    update_SRCC_win_list = np.zeros((NUM_REPETITIONS, num_epochs))
    update_KRCC_win_list = np.zeros((NUM_REPETITIONS, num_epochs))
    update_CoD_win_list = np.zeros((NUM_REPETITIONS, num_epochs))

    update_PLCC_seq_list = np.zeros((NUM_REPETITIONS, num_epochs))
    update_SRCC_seq_list = np.zeros((NUM_REPETITIONS, num_epochs))
    update_KRCC_seq_list = np.zeros((NUM_REPETITIONS, num_epochs))
    update_CoD_seq_list = np.zeros((NUM_REPETITIONS, num_epochs))

    #####################################################################################################################
    valid_PLCC_win_list = np.zeros((NUM_REPETITIONS, num_epochs))
    valid_SRCC_win_list = np.zeros((NUM_REPETITIONS, num_epochs))
    valid_KRCC_win_list = np.zeros((NUM_REPETITIONS, num_epochs))
    valid_CoD_win_list = np.zeros((NUM_REPETITIONS, num_epochs))

    valid_PLCC_seq_list = np.zeros((NUM_REPETITIONS, num_epochs))
    valid_SRCC_seq_list = np.zeros((NUM_REPETITIONS, num_epochs))
    valid_KRCC_seq_list = np.zeros((NUM_REPETITIONS, num_epochs))
    valid_CoD_seq_list = np.zeros((NUM_REPETITIONS, num_epochs))

    for rep in range(NUM_REPETITIONS):
        update_barometer, valid_barometer = train_model(exp_name, rep + 1)

        update_PLCC_win_list[rep, :] = update_barometer['PLCC_win']
        update_SRCC_win_list[rep, :] = update_barometer['SRCC_win']
        update_KRCC_win_list[rep, :] = update_barometer['KRCC_win']
        update_CoD_win_list[rep, :] = update_barometer['CoD_win']

        update_PLCC_seq_list[rep, :] = update_barometer['PLCC_seq']
        update_SRCC_seq_list[rep, :] = update_barometer['SRCC_seq']
        update_KRCC_seq_list[rep, :] = update_barometer['KRCC_seq']
        update_CoD_seq_list[rep, :] = update_barometer['CoD_seq']

        print_str = f'\n(Win-level) Average of {rep + 1}/{NUM_REPETITIONS} reps : PLCC / SRCC / KRCC / CoD ='
        print_str += f' {update_PLCC_win_list[:rep + 1, -1].mean():.4f}±{update_PLCC_win_list[:rep + 1, -1].std():.4f}'
        print_str += f' {update_SRCC_win_list[:rep + 1, -1].mean():.4f}±{update_SRCC_win_list[:rep + 1, -1].std():.4f}'
        print_str += f' {update_KRCC_win_list[:rep + 1, -1].mean():.4f}±{update_KRCC_win_list[:rep + 1, -1].std():.4f}'
        print_str += f' {update_CoD_win_list[:rep + 1, -1].mean():.4f}±{update_CoD_win_list[:rep + 1, -1].std():.4f}'
        print(print_str)

        print_str = f'(Seq-level) Average of {rep + 1}/{NUM_REPETITIONS} reps : PLCC / SRCC / KRCC / CoD ='
        print_str += f' {update_PLCC_seq_list[:rep + 1, -1].mean():.4f}±{update_PLCC_seq_list[:rep + 1, -1].std():.4f}'
        print_str += f' {update_SRCC_seq_list[:rep + 1, -1].mean():.4f}±{update_SRCC_seq_list[:rep + 1, -1].std():.4f}'
        print_str += f' {update_KRCC_seq_list[:rep + 1, -1].mean():.4f}±{update_KRCC_seq_list[:rep + 1, -1].std():.4f}'
        print_str += f' {update_CoD_seq_list[:rep + 1, -1].mean():.4f}±{update_CoD_seq_list[:rep + 1, -1].std():.4f}'
        print(print_str)

        #####################################################################################################################
        valid_PLCC_win_list[rep, :] = valid_barometer['PLCC_win']
        valid_SRCC_win_list[rep, :] = valid_barometer['SRCC_win']
        valid_KRCC_win_list[rep, :] = valid_barometer['KRCC_win']
        valid_CoD_win_list[rep, :] = valid_barometer['CoD_win']

        valid_PLCC_seq_list[rep, :] = valid_barometer['PLCC_seq']
        valid_SRCC_seq_list[rep, :] = valid_barometer['SRCC_seq']
        valid_KRCC_seq_list[rep, :] = valid_barometer['KRCC_seq']
        valid_CoD_seq_list[rep, :] = valid_barometer['CoD_seq']

        print_str = f'\n(Win-level) Average of {rep + 1}/{NUM_REPETITIONS} reps : PLCC / SRCC / KRCC / CoD ='
        print_str += f' {valid_PLCC_win_list[:rep + 1, -1].mean():.4f}±{valid_PLCC_win_list[:rep + 1, -1].std():.4f}'
        print_str += f' {valid_SRCC_win_list[:rep + 1, -1].mean():.4f}±{valid_SRCC_win_list[:rep + 1, -1].std():.4f}'
        print_str += f' {valid_KRCC_win_list[:rep + 1, -1].mean():.4f}±{valid_KRCC_win_list[:rep + 1, -1].std():.4f}'
        print_str += f' {valid_CoD_win_list[:rep + 1, -1].mean():.4f}±{valid_CoD_win_list[:rep + 1, -1].std():.4f}'
        print(print_str)

        print_str = f'(Seq-level) Average of {rep + 1}/{NUM_REPETITIONS} reps : PLCC / SRCC / KRCC / CoD ='
        print_str += f' {valid_PLCC_seq_list[:rep + 1, -1].mean():.4f}±{valid_PLCC_seq_list[:rep + 1, -1].std():.4f}'
        print_str += f' {valid_SRCC_seq_list[:rep + 1, -1].mean():.4f}±{valid_SRCC_seq_list[:rep + 1, -1].std():.4f}'
        print_str += f' {valid_KRCC_seq_list[:rep + 1, -1].mean():.4f}±{valid_KRCC_seq_list[:rep + 1, -1].std():.4f}'
        print_str += f' {valid_CoD_seq_list[:rep + 1, -1].mean():.4f}±{valid_CoD_seq_list[:rep + 1, -1].std():.4f}'
        print(print_str)
